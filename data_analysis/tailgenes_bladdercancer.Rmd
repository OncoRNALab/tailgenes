...
#data.table::fwrite(genes_ens,file="ensID_biotype.txt", sep="\t",quote =F)
...
gene_sum <- pc_gene_counts %>% pivot_longer(names_to="UniqueID", values_to="counts",-"gene_id") %>% filter(counts>=10) %>% group_by(UniqueID) %>% summarise(nr_genes=n()) %>% arrange(nr_genes)
...
#summary(tmp)
data.table::fwrite(tail_genes, file="../data/tail_genes/bladdercancer_tailgenes.txt",sep="\t", row.names = F, quote=F, na="")
...
```{r}
#z_scores_LOO <- data.table::fread("../data/tail_genes/bladdercancer_zscores_tail.txt",data.table = F)
tail_genes <- data.table::fread("../data/tail_genes/bladdercancer_tailgenes.txt",data.table=F)
...
rm(tail_genes_random,z_scores_LOO_random, tail_genes, tail_genes_ndev)
...
(10x) 5-fold CV from start (now with FT both on counts and z) + assess performance according to sex
```{r, 5fold CV bladdercancer, message=F}
sample_annotation <- data.table::fread("../input/bladdercancer_sample_annotation.txt", sep="\t", quote="",header=T, data.table=FALSE) %>% mutate(UniqueID = paste0(Abbreviation,"_",ReplicateNr,"_",Cohort))
...
for (repeat_x in 1:num_repeats) {
  ...
  sample_annotation$cancer_binary <- ifelse(sample_annotation$Cancer=="control",0,1)
  ...
  for(i in 1:num_folds) {
    ...
    z_scores <- data.frame(gene_id = bladdercancer_normcounts %>% pull(gene_id))
    ##reference only based on CONTROLS in training except sample itself
    for (samp in (colnames(bladdercancer_normcounts)[-1])) {
      #print(samp)
      z_scores_sample <- tail_z_score(bladdercancer_normcounts %>%
                                            dplyr::select(c("gene_id",all_of(samp))) %>%
                                            column_to_rownames("gene_id"),
                                          bladdercancer_normcounts %>% dplyr::select(c("gene_id",grep("CONTROL",trainIDs,value =T))) %>% #only training set CONTROL samples
                                            dplyr::select(-any_of(samp)) %>% #remove sample of interest from reference
                                            column_to_rownames("gene_id"))
    ...
    test <- reduced_z_table %>% pivot_longer(names_to="sample", values_to="zscores",-"gene_id") %>% 
        ...
        left_join(bladdercancer_normcounts %>% #add normalized counts
    ...
    tail_genes <- tail_genes %>% 
      ...
      full_join(sample_annotation %>% filter(Cohort=="bladdercancer") %>% dplyr::select(c("sample"=UniqueID)), by=c("sample")) %>% mutate(n_dev=ifelse(is.na(n_dev),0,n_dev)) 
    
    #### Model training and testing (x repeats in y-fold cross-validation!)
    ### train with training, test on testing data
    data <- tail_genes %>% mutate(disease=gsub("_.*","",sample)) %>% mutate(cancer=ifelse(disease=="CONTROL", F, T)) %>% 
    ...
    rm(z_scores, z_scores_Tail, data, train_data, test_data, prob_predictions, tail_genes, roc_curve, gene_list, trainIDs, testIDs, samp, roc_curve)
...
roc_data_bladdercancer_5fCV <- rbind(roc_data, mean_roc_data)
...
BTG_AUC_CV <- data.frame(
    comparison="bladdercancervsCTRL",
    CV = "start",
...
write.table(BTG_AUC_CV,file="../data/tail_genes/BTG_AUC_bladdercancer_5fCV.txt", row.names = F, , col.names=T, quote=F, na="")
  
write.table(roc_data_bladdercancer_5fCV, file=paste0("../data/tail_genes/ROC_bladdercancer_5fCV.txt"), row.names = F, , col.names=T, quote=F, na="")
  
saveRDS(consensus_genes, file=paste0("../data/tail_genes/BTGidentity_bladdercancer_5fCV.RData"))
  #consensus_genes <- readRDS(paste0("../data/tail_genes/BTG_identity_bladdercancer_5fCV.RData"))
saveRDS(BTGpersample, file=paste0("../data/tail_genes/BTGpersample_bladdercancer_5fCV.RData"))


rm(list=c("test_indices", "all_auc_values", "bladdercancer_normcounts", "sample_annotation", "repeat_x", "mean_auc", "num_folds", "consensus_genes_100","i", "all_auc_values"))
...
for (cancer in c("bladdercancer")){
  print(cancer)
  ROC_curves_all <- plyr::rbind.fill(ROC_curves_all, data.table::fread(file=paste0("../data/tail_genes/ROC_",cancer,"_5fCV.txt"), fill=T, data.table=F) %>% mutate(cohort="bladdercancer",comparison=paste0(cancer,"vsCTRL"),cancertype=cancer))
  
  BTG_AUC_all <- plyr::rbind.fill(BTG_AUC_all, data.table::fread(file=paste0("../data/tail_genes/BTG_AUC_",cancer,"_5fCV.txt"),fill=T, data.table=F) %>% mutate(cohort="bladdercancer"))
...
print(ggplot(ROC_curves_all %>% filter(Fold=="mean"),
    ...
    scale_color_manual(values=c(bladdercancer="#009988")) +
        
        geom_abline(intercept=0, slope=1, linetype="dashed", linewidth=0.5, color="grey"))

ggsave(filename = "../figures/bladdercancer_biomarkertail_ROC_5fCV.pdf", plot=last_plot(), width=5.8, height=6.1, units="cm")
...
```{r}
sample_annotation <- data.table::fread("../input/bladdercancer_sample_annotation.txt", sep="\t", quote="",header=T, data.table=FALSE) %>% filter(Cohort=="bladdercancer") %>% mutate(UniqueID = paste0(Abbreviation,"_",ReplicateNr,"_",Cohort)) 
...
my_comparisons <- list( c("CONTROL", "bladdercancer") )
...
tail_genes <- left_join(z_scores_LOO %>% pivot_longer(names_to="sample", values_to="zscores",-"gene_id") %>% filter(!is.na(zscores)) %>% filter(abs(zscores)>3), bladdercancer_normcounts %>% #add normalized counts             
...
for (cancer in c("bladdercancer")) {
  BTG_consensus_50_persample <- rbind(BTG_consensus_50_persample, 
    ...
    filter(Abbreviation %in% c("CONTROL","BLCA")) %>%
    group_by(sample) %>% dplyr::summarise(n_dev=n()) %>%
    #make sure all samples are present
    full_join(sample_annotation %>% filter(Abbreviation %in% c("CONTROL","BLCA")) %>% dplyr::select(c("sample"=UniqueID, "disease"=Abbreviation)), by=c("sample")) %>% mutate(n_dev=ifelse(is.na(n_dev),0,n_dev)) %>%
    mutate(comparison=paste0(cancer,"vsCtrl"),disease=factor(disease, levels=c("CONTROL","BLCA")), 
           cancer=ifelse(disease=="CONTROL", F, T)) %>% 
      mutate(disease=factor(disease)) %>%
      mutate(disease=relevel(disease,ref="CONTROL")))
}

BTG_consensus_50_persample <- BTG_consensus_50_persample %>% mutate(comparison=factor(comparison, levels=c("bladdercancervsCtrl")))
...
data_tmp <- BTG_consensus_50_persample %>% mutate(disease=paste(disease)) #mutate disease column to string for wilcox_test
rstatix::wilcox_test(data=data_tmp %>% filter(comparison=="bladdercancervsCtrl"), n_dev ~ disease)
rstatix::wilcox_effsize(data=data_tmp %>% filter(comparison=="bladdercancervsCtrl"), n_dev ~ disease)
#rstatix::wilcox_effsize(data=data_tmp %>% filter(comparison=="bladdercancervsCtrl"), n_dev ~ disease,ci=T)
...
sapply(BTG_genes[["bladdercancer"]], count_words) %>% unlist() %>% summary()


#Overlap of unique biomarker tail genes in different types and differentially abundant mRNAs across types
DAA_genes <- data.table::fread("../data/differentialabundance_bladdercancer.txt", data.table = F) %>% mutate(DA=ifelse((padj<0.05) & (log2FoldChange>1), "higher", ifelse((padj<0.05) & (log2FoldChange<(-1)), "lower", "no")))  #higher/lower: differentially abundant genes (DA)

table((BTG_consensus_50[["bladdercancer"]]) %in% (DAA_genes %>% filter(DA!="no") %>% pull(gene_id) %>% unique()))
```
...
#data.table::fwrite(genes_ens_full,file="ensID_genename.txt", sep="\t",quote =F)


biomarker_overview = data.frame(ensembl_gene_id=c( BTG_consensus_50[["bladdercancer"]]), biomarkerset=c(rep("BLCA vs CONTROL",length(BTG_consensus_50[["bladdercancer"]])))) %>% left_join(genes_ens_full)
data.table::fwrite(biomarker_overview %>% dplyr::select(c("set"="biomarkerset","ensembl_gene"="ensembl_gene_id","gene_name"="external_gene_name")),file = "../data/bladdercancer_biomarkertail_list_5fCV_consensus50.txt", sep="\t",quote=F,row.names=F)
```
...
Test whether there are sex-linked tail genes
- make reference consisting of 1 sex, determine tail genes in samples from other sex
```{r, eval=F}
### male reference and determine tail genes in female samples (for check: if you determine them in the male samples from reference, none identified)
z_scores_maleref <- data.frame(gene_id = bladdercancer_normcounts %>% pull(gene_id))
for (samp in (all_of(sample_annotation %>% filter(Abbreviation=="CONTROL") %>% filter(Sex=="F") %>% pull(UniqueID)))) {
  #print(samp)
  z_scores_maleref_sample <- tail_z_score(bladdercancer_normcounts %>% dplyr::select(c("gene_id",all_of(samp))) %>% column_to_rownames("gene_id"),
                                                                bladdercancer_normcounts %>% dplyr::select(c("gene_id",all_of(sample_annotation %>% filter(Abbreviation=="CONTROL") %>% filter(Sex=="M") %>% pull(UniqueID)))) %>% 
                                                  #dplyr::select(-any_of(samp)) %>% #remove sample of interest from reference #reference is always based on other samples other sex
                                                  column_to_rownames("gene_id"))
  
  z_scores_maleref <- left_join(z_scores_maleref, z_scores_maleref_sample %>% rownames_to_column("gene_id"), by="gene_id")
}
rm(z_scores_maleref_sample)

gene_list <- vector(mode="list")
### overview tail genes
tail_genes <- left_join(z_scores_maleref %>% pivot_longer(names_to="sample", values_to="zscores",-"gene_id") %>% filter(!is.na(zscores)) %>% filter(abs(zscores)>3), bladdercancer_normcounts %>% #add normalized counts             
pivot_longer(names_to="sample",values_to="ncounts",-"gene_id"), by=c("gene_id"="gene_id","sample")) %>% filter(ncounts>=40) 

tail_genes_female <- tail_genes$gene_id %>% unique() #474 female tail genes

length(intersect(consensus_union, tail_genes_female)) #62 overlap


### same but now female reference and determine tail genes in male samples (for check: if you determine them in the female samples from reference, none identified)
z_scores_femaleref <- data.frame(gene_id = bladdercancer_normcounts %>% pull(gene_id))
for (samp in (all_of(sample_annotation %>% filter(Abbreviation=="CONTROL") %>% filter(Sex=="M") %>% pull(UniqueID)))) {
  #print(samp)
  z_scores_femaleref_sample <- tail_z_score(bladdercancer_normcounts %>% dplyr::select(c("gene_id",all_of(samp))) %>% column_to_rownames("gene_id"),
                                                                bladdercancer_normcounts %>% dplyr::select(c("gene_id",all_of(sample_annotation %>% filter(Abbreviation=="CONTROL") %>% filter(Sex=="F") %>% pull(UniqueID)))) %>% 
                                                  #dplyr::select(-any_of(samp)) %>% #remove sample of interest from reference #reference is always based on other samples other sex
                                                  column_to_rownames("gene_id"))
  
  z_scores_femaleref <- left_join(z_scores_femaleref, z_scores_femaleref_sample %>% rownames_to_column("gene_id"), by="gene_id")
}
rm(z_scores_femaleref_sample)

gene_list <- vector(mode="list")
### overview tail genes
tail_genes <- left_join(z_scores_femaleref %>% pivot_longer(names_to="sample", values_to="zscores",-"gene_id") %>% filter(!is.na(zscores)) %>% filter(abs(zscores)>3), bladdercancer_normcounts %>% #add normalized counts             
pivot_longer(names_to="sample",values_to="ncounts",-"gene_id"), by=c("gene_id"="gene_id","sample")) %>% filter(ncounts>=40) 

tail_genes_male <- tail_genes$gene_id %>% unique() #282 male tail genes

length(intersect(consensus_union, tail_genes_male)) #7 overlap

length(intersect(tail_genes_male, tail_genes_female)) #34 overlap
...
#for (i in 1:2){
  ...
  bladdercancer_normcounts <- data.table::fread("../data/bladdercancer_normcounts.txt", data.table=F) #start from original normalized counts
  ...
  write.table(BTG_AUC_CV,file=paste0("../data/tail_genes/labelswap/BTG_AUC_bladdercancer_5fCV_labelswap-",swap_x,".txt"), row.names = F, , col.names=T, quote=F, na="")
  
  #write.table(roc_data_bladdercancer_5fCV, file=paste0("../data/tail_genes/labelswap/ROC_bladdercancer_5fCV_labelswap-",swap_x,".txt"), row.names = F, , col.names=T, quote=F, na="")
  
  saveRDS(consensus_genes, file=paste0("../data/tail_genes/labelswap/BTGidentity_bladdercancer_5fCV_labelswap-",swap_x,".RData"))
  saveRDS(BTGpersample, file=paste0("../data/tail_genes/labelswap/BTGpersample_bladdercancer_5fCV_labelswap-",swap_x,".RData"))
...
rm(tail_genes,z_scores_LOO, tail_genes, tail_genes_ndev)
rm(list=c("test_indices", "all_auc_values", "bladdercancer_normcounts", "sample_annotation", "repeat_x", "mean_auc", "num_folds", "consensus_genes_100","swap_x","i", "all_auc_values"))
